...
macro_rules! full_font {
    ($bytes:expr) => {{
        ...
        unsafe { $crate::full::FullFont::from_static_bytes(&FONT_BYTES.0) }
    }};
    ($bytes:expr, iwram) => {{
        #[repr(C, align(4))]
        struct AlignedFont([u8; { $bytes.len() }]);
...
impl FullFont {
    /// Parse a mode-1 binary font blob. Panics if the data has the wrong mode byte or its
    /// length does not match the glyph dimensions in the header
    ///
    /// # Safety
    ///
    /// `bytes` must be 4-byte aligned. The `full_font!` and `include_agb_font!` macros
    ...
    pub const unsafe fn from_static_bytes(bytes: &'static [u8]) -> Self {
        ...
        assert!(
            data_len == glyph_size * GLYPH_COUNT_FULL * 4,
